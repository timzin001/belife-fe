...
const changeName = (evt: any) => {
  CreateDepartmentValidate.name(instance, t, $orgAPI)
}
...
const clickSave = async (evt: any) => {
  const validate = await CreateDepartmentValidate.all(instance, t, $orgAPI)
  if (!validate) {
    return
  }
...
@import url('~/assets/scss/org/CreateDepartment.scss');
</style>
