...
/// Import
import type { UpdateAddressData } from '~/types/user/ProfileType'
...
const instance = ref<UpdateAddressData>({
  titleDialog: t('update_address'),
  address: props.address,
})

/// Method
const clickSave = async () => {
  const validate = ProfileValidate.address(instance, t)
  if (!validate) {
    return
  }
  /// Show loading
  ...
  const { data, error, status } = await CallAPI(
    APIPathUser.POST_UPDATE_ADDRESS_USER,
    options,
    toast,
    t,
    false
...
const changeAddress = (evt: any) => {
  ProfileValidate.address(instance, t)
}
...
watch(
  ...
  (value) => {
    /// Update address
    instance.value.address = props.address
  }
)
onMounted(() => {})
</script>
